...
func (s *Shader) compile() error {
	...
	vs := mylog.Check2(s.graphics.context.newShader(gl.VERTEX_SHADER, vssrc))

	defer s.graphics.context.ctx.DeleteShader(uint32(vs))

	fs := mylog.Check2(s.graphics.context.newShader(gl.FRAGMENT_SHADER, fssrc))

	defer s.graphics.context.ctx.DeleteShader(uint32(fs))

	p := mylog.Check2(s.graphics.context.newProgram([]shader{vs, fs}, theArrayBufferLayout.names()))
