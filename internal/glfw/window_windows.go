...
func CreateWindow(width, height int, title string, monitor *Monitor, share *Window) (window *Window, ferr error) {
	...
	if mylog.Check(checkValidContextConfig(&ctxconfig)); err != nil {
		return nil, err
	}
	...
	if mylog.Check(window.platformCreateWindow(&wndconfig, &ctxconfig, &fbconfig)); err != nil {
		return nil, err
	}
...
func (w *Window) Destroy() error {
	...
	if uintptr(unsafe.Pointer(w)) == current {
		if mylog.Check((*Window)(nil).MakeContextCurrent()); err != nil {
			return err
		}
	}
	...
	if mylog.Check(w.platformDestroyWindow()); err != nil {
		return err
	}
...
func (w *Window) SetTitle(title string) error {
	...
	if mylog.Check(w.platformSetWindowTitle(title)); err != nil {
		return err
	}
	return nil
}
...
func (w *Window) SetIcon(images []image.Image) error {
	...
	if mylog.Check(w.platformSetWindowIcon(gimgs)); err != nil {
		return err
	}
	return nil
}
...
func (w *Window) SetPos(xpos, ypos int) error {
	...
	if mylog.Check(w.platformSetWindowPos(xpos, ypos)); err != nil {
		return err
	}
	return nil
}
...
func (w *Window) SetSize(width, height int) error {
	...
	if mylog.Check(w.platformSetWindowSize(width, height)); err != nil {
		return err
	}
	return nil
}
...
func (w *Window) SetSizeLimits(minwidth, minheight, maxwidth, maxheight int) error {
	...
	if mylog.Check(w.platformSetWindowSizeLimits(minwidth, minheight, maxwidth, maxheight)); err != nil {
		return err
	}
...
func (w *Window) SetAspectRatio(numer, denom int) error {
	...
	if mylog.Check(w.platformSetWindowAspectRatio(numer, denom)); err != nil {
		return err
	}
	return nil
}
...
func (w *Window) SetOpacity(opacity float32) error {
	...
	if mylog.Check(w.platformSetWindowOpacity(opacity)); err != nil {
		return err
	}
	return nil
}
...
func (w *Window) Maximize() error {
	...
	if mylog.Check(w.platformMaximizeWindow()); err != nil {
		return err
	}
	return nil
}
...
func (w *Window) GetAttrib(attrib Hint) int {
	...
	switch attrib {
	case Focused:
		return boolToInt(w.platformWindowFocused())
	case Iconified:
		return boolToInt(w.platformWindowIconified())
	case Visible:
		return boolToInt(w.platformWindowVisible())
	case Maximized:
	...
	case MousePassthrough:
		return boolToInt(w.mousePassthrough)
	case TransparentFramebuffer:
		return boolToInt(w.platformFramebufferTransparent())
	case Resizable:
		return boolToInt(w.resizable)
	case Decorated:
		return boolToInt(w.decorated)
	case Floating:
		return boolToInt(w.floating)
	case AutoIconify:
		return boolToInt(w.autoIconify)
	case ClientAPI:
		return w.context.client
	case ContextCreationAPI:
		return w.context.source
	case ContextVersionMajor:
		return w.context.major
	case ContextVersionMinor:
		return w.context.minor
	case ContextRevision:
		return w.context.revision
	case ContextRobustness:
		return w.context.robustness
	case OpenGLForwardCompat:
		return boolToInt(w.context.forward)
	case OpenGLDebugContext:
		return boolToInt(w.context.debug)
	case OpenGLProfile:
		return w.context.profile
	case ContextReleaseBehavior:
		return w.context.release
	case ContextNoError:
...
func (w *Window) SetAttrib(attrib Hint, value int) error {
	...
	case Resizable:
		...
		w.resizable = bValue
		if w.monitor == nil {
			if mylog.Check(w.platformSetWindowResizable(bValue)); err != nil {
				return nil
			}
		}
		return nil
	case Decorated:
		...
		w.decorated = bValue
		if w.monitor == nil {
			if mylog.Check(w.platformSetWindowDecorated(bValue)); err != nil {
				return err
			}
		}
		return nil
	case Floating:
		...
		w.floating = bValue
		if w.monitor == nil {
			if mylog.Check(w.platformSetWindowFloating(bValue)); err != nil {
				return err
			}
		}
		return nil
	...
	case MousePassthrough:
		...
		if mylog.Check(w.platformSetWindowMousePassthrough(bValue)); err != nil {
			return err
		}
		return nil
	default:
...
func (w *Window) SetMonitor(monitor *Monitor, xpos, ypos, width, height, refreshRate int) error {
	...
	if mylog.Check(w.platformSetWindowMonitor(monitor, xpos, ypos, width, height, refreshRate)); err != nil {
		return err
	}
	return nil
}
...
func WaitEvents() error {
	...
	if mylog.Check(platformWaitEvents()); err != nil {
		return err
	}
	return nil
}
...
func WaitEventsTimeout(timeout float64) error {
	...
	if mylog.Check(platformWaitEventsTimeout(timeout)); err != nil {
		return err
	}
	return nil
}
...
func PostEmptyEvent() error {
	...
	if mylog.Check(platformPostEmptyEvent()); err != nil {
		return err
	}
	return nil
}
